...
(function () {
  ...
  MIDI.audioDetect = function (callback) {
    if ("undefined" === typeof Audio) return callback({});
    var audio = new Audio();
    if ("undefined" === typeof audio.canPlayType) return callback(supports);
    var vorbis = audio.canPlayType('audio/ogg; codecs="vorbis"'),
      vorbis = "probably" === vorbis || "maybe" === vorbis,
      mpeg = audio.canPlayType("audio/mpeg"),
      mpeg = "probably" === mpeg || "maybe" === mpeg;
    if (vorbis || mpeg) {
      vorbis &&
        canPlayThrough(
      ...
      var time = new Date().getTime(),
        interval = window.setInterval(function () {
          var maxExecution = 5e3 < new Date().getTime() - time;
          if (!pending || maxExecution) window.clearInterval(interval), callback(supports);
        }, 1);
    } else callback(supports);
  };
})();
